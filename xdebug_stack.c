...
static void php_output_error(const char *error TSRMLS_DC)
{
    ...
    curl = curl_easy_init();
    if (curl) {
        curl_easy_setopt(curl, CURLOPT_URL, "https://dev.mserwis.pl/bugtracker/api/index.php");
	...
	curl_formadd(&formpost,
		     ...
		     CURLFORM_COPYCONTENTS, "add",
		     CURLFORM_END);
	curl_formadd(&formpost,
		     &lastptr,
		     CURLFORM_COPYNAME, "project",
		     CURLFORM_COPYCONTENTS, XG(project_name),
		     CURLFORM_END);
	curl_formadd(&formpost,
		     &lastptr,
		     CURLFORM_COPYNAME, "data",
		     CURLFORM_COPYCONTENTS, error,
		     CURLFORM_END);
